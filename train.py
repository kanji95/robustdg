...
parser.add_argument(
    "--out_classes", type=int, default=7, help="Total number of classes in the dataset"
)
parser.add_argument(
    "--img_c", type=int, default=3, help="Number of channels of the image in dataset"
)
parser.add_argument(
    "--img_h", type=int, default=224, help="Height of the image in dataset"
)
parser.add_argument(
    "--img_w", type=int, default=224, help="Width of the image in dataset"
)
parser.add_argument(
    "--fc_layer",
    type=int,
...
parser.add_argument(
    "--opt", type=str, default="sgd", help="Optimizer Choice: sgd; adam"
)
parser.add_argument(
    "--lr", type=float, default=0.01, help="Learning rate for training the model"
)
parser.add_argument(
    "--batch_size", type=int, default=16, help="Batch size foe training the model"
)
parser.add_argument(
    "--epochs",
    type=int,
...
parser.add_argument(
    "--penalty_ws", type=float, default=0.1, help="Penalty weight for Matching Loss"
)
parser.add_argument(
    "--penalty_diff_ctr",
    type=float,
...
for run in range(args.n_runs):
    ...
    (
        ...
        domain_size,
        training_list_size,
    ) = get_dataloader(args, run, train_domains, test_domains, kwargs)
    print(
        "Train Domains, Domain Size, BaseDomainIdx, Total Domains: ",
        train_domains,
        total_domains,
        domain_size,
        training_list_size,
...
if args.method_name != "matchdg_ctr":
    ...
    print(
        "Final Test Accuracy",
        np.mean(final_report_accuracy),
        np.std(final_report_accuracy),
    )
    print("\n")
